...
Quickshell projects use `Main.qml` plus protected `challenge_test.qml` cases. The owned runner in `gym/runtime/quickshell-runner.qml` runs real components offscreen and accepts only observed passing cases with a completed run. Validate every starter and a private working candidate using an installed Quickshell and Bubblewrap; keep reference implementations outside tracked curriculum. Core CI checks the runner/reporting contract without requiring Quickshell on Ubuntu.
